...
def updateBoard(board: Map[Int, List[Char]], move: Move): Unit = {
    val from = move.from
    val to = move.to
    val stack = board(from).splitAt(move.count)

    board(from) = stack._2
    board(to) = stack._1.reverse ::: board(to)
}
...
@main def template(fileName: String) = {
    ...
    lines.map(line => parseMove(line))
        .foreach(move => updateBoard(board, move))
